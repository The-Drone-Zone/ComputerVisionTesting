...
class SSD(nn.Module):
    ...
    def load(self, model):
        ...
        # Modified Stack Overflow version
        # current_model_dict = self.state_dict()
        # # loaded_state_dict = torch.load(model, map_location=torch.device('cpu'))
        # loaded_state_dict = torch.load(model, map_location=lambda storage, loc: storage)
